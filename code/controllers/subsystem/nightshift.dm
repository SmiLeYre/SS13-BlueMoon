...
/datum/controller/subsystem/nightshift/proc/update_nightshift(active, announce = TRUE, max_level_override)
	...
	if(announce)
		if (active)
			announce("Добрый вечер, Команда Космической Станции. Чтобы снизить потребление энергии и стимулировать Циркадные Ритмы некоторых видов, все огни на борту станции были приглушены на ночь.")
		else
			announce("Доброе утро, Команда Космической Станции. Поскольку сейчас дневное время, все огни на борту станции были восстановлены до их прежней яркости.")
	var/max_level
